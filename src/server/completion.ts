...
export function completion(model: SetModel, uri: string, idx: ComponentsIndex, pos: Position): CompletionItem[] {
  ...
  if (cls && segs.length === 1) {
    return idx.components[cls].map((c) => ({
      ...
      documentation: c.description ? { kind: "markdown" as const, value: c.description } : undefined,
    }));
  }
  ...
  if (segs[0] === "service" && segs[1] === "pipelines" && segs.length >= 4) {
    ...
    const refCls: ComponentClass | undefined =
      bucket === "receivers" ? "receiver" : bucket === "processors" ? "processor" : bucket === "exporters" ? "exporter" : undefined;
    if (!refCls) return [];
  ...
  // Top-level keys.
  if (segs.length === 0) {
    return ["receivers", "processors", "exporters", "connectors", "extensions", "service"].map((k) => ({
      label: k,
